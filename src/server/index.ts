...
app.get("/callback", async (req, res) => {
  ...
  const session = await prisma.authSession.findUnique({ where: { discordId: state as string } });
  if (!session) return res.status(400).send("Invalid or expired session");

  try {
    const tokens = await exchangeCodeForTokens(code as string, session.verifier);
    ...
    await prisma.authSession.delete({ where: { discordId: session.discordId } });
